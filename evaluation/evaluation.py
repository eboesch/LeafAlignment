"""script for evaluating pairwise registration"""
import os
import csv
import kornia as K
import numpy as np
...
from leafalignment.metrics import mse_masked, local_ncc_masked, nmi_masked, ssim_masked, iou, hausdorff
...
if __name__ == "__main__":
    out_file = "../results/registration_eval.csv"
    ...
    METHODS = ["Piecewise Affine", "LoFTR + TPS ROI", "LoFTR + TPS ROI with Markers", "LoFTR + TPS ROI Pre-Rotated", "LoFTR + TPS ROI Pre-Rotated with Markers", "LoFTR + TPS Full", "LoFTR + TPS Full with Markers", "LoFTR + TPS Full Pre-Rotated", "LoFTR + TPS Full with Markers Pre-Rotated"]
    data_to_load = []
    if "Piecewise Affine" in METHODS:
        data_to_load.extend(['target_images', 'target_masks'])
    if any(m.startswith("LoFTR + TPS ROI") for m in METHODS):
        data_to_load.extend(['roi_leaf_images', 'roi_leaf_masks'])
    if any(m.startswith("LoFTR + TPS Full") for m in METHODS):
